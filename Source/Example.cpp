...
char     g_szPluginName[]     = "Display Plugin by Zeffrin";
...
const char*    InternalsPluginInfo::GetDesc()     const { return "Display Info on secondary display"; }
...
const char ExampleInternalsPlugin::m_szName[] = "DisplayPlugin";
...
void ExampleInternalsPlugin::WriteToLogFile( const char * const openStr, const char * const msg )
{
  FILE *fo;

  fo = fopen( "DisplayPluginLog.txt", openStr );
  if( fo != NULL )
  {
    fprintf( fo, "%s\n", msg );
    fclose( fo );
...
void ExampleInternalsPlugin::Startup()
{
  ...
  // example output data files.
	...
	if (hMapHandle == NULL) 
	{ 
		WriteToLogFile("w", "-Unable to connect to Display-");
		return;
	}
   ...
   if (TelemBuf == NULL) 
   { 
      WriteToLogFile("w", "-Unable to map view of file-");
	  CloseHandle(hMapHandle);
	  hMapHandle = NULL;
      return;
   }

   hEventHandle = OpenEvent(EVENT_MODIFY_STATE, FALSE, szEventName);
   if (hEventHandle == NULL)
   {
    ...
    Connected = true;
	WriteToLogFile( "w", "-Display Plugin Connected-" );
...
void ExampleInternalsPlugin::Shutdown()
{
	if(Connected)
	{
	    ...
	    hEventHandle = NULL;
	    hMutex = NULL;
		WriteToLogFile("a", "Disconnected from Shared Memory");
		Connected = false;
	}
}
